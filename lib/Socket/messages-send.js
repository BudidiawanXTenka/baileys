...
const makeMessagesSocket = (config) => {
    ...
    const tenkaInstance = new tenka(Utils_1, waUploadToServer, relayMessage);
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            const messageType = tenkaInstance.detectType(content);
            ...
            if (messageType) {
                switch(messageType) {
                    case 'PAYMENT':
                        const paymentContent = await tenkaInstance.handlePayment(content, quoted);
                    ...
                    case 'PRODUCT':
                        const productContent = await tenkaInstance.handleProduct(content, jid, quoted);
                    ...
                    case 'INTERACTIVE':
                        const interactiveContent = await tenkaInstance.handleInteractive(content, jid, quoted);
                    ...
                    case 'ALBUM':
                        return await tenkaInstance.handleAlbum(content, jid, quoted)
                    case 'EVENT':
                        return await tenkaInstance.handleEvent(content, jid, quoted)
                    case 'POLL_RESULT':
                        return await tenkaInstance.handlePollResult(content, jid, quoted)
                    case 'GROUP_STORY':
                        return await tenkaInstance.handleGroupStory(content, jid, quoted)
                    case 'STICKER_PACK':
                        const packContent = await tenkaInstance.handleStickerPack(content, jid, quoted);
